...
\begin{subsecframe}{User libraries, plugins}
  ...
  \item \name{Why3}: call SMT solvers from \Coq
  \end{itemize}
...
\begin{subsecframe}{The bug}
  ...
  \item Due to a (very) extensional axiom, related to \emph{equality} on
    types interacting with the guard condition.
  \item A good fix is known, but breaks the stdlib's use of the smart
    ...
    \name{B. Barras, C. Paulin, M. Dénès, P. Boutilier}, $\ldots$
  \item[=] Happens in \name{Agda} too!
...
\begin{subsecframe}{Tentative planning}
  \begin{itemize}
  \item February 2014: 8.5-α release and 8.4pl6 (or 8.4-2) (\TODO{MS:n bugs closed})
...
\begin{subsecframe}{CoqMT -- \name{P.Y. Strub} et al}
  ...
  \item[$"->"$] A \emph{strict} equality avoiding coercions, a slightly
    larger TCB (SN proof available).
...
\begin{subsecframe}{\Coq as a programming language}
  ...
  \begin{itemize}
  \item I/O, FFI (\TODO{MD: Demo Hello world! in Coq})
