...
\newenvironment{subsecframe}[1]{\begin{subsection}{#1}\begin{frame}\frametitle{#1}}{\end{frame}\end{subsection}}

\newenvironment{subsecframefragile}[1]{\begin{frame}[fragile,environment=subsecframefragile]\frametitle{#1}\begin{subsection}{#1}}{\end{subsection}\end{frame}}

\newcommand{\demoframe}[2]{\begin{frame}\begin{center}{\Huge #1}\\\bigskip{\Large #2}\end{center}\end{frame}}
...
\demoframe{Incremental development}{Enrico Tassi}
...
\demoframe{New proof engine}{Arnaud Spiwack}
...
\demoframe{Universe polymorphism}{Matthieu Sozeau}
...
\demoframe{Native compilation}{Maxime Dénès \& Benjamin Grégoire}
...
\demoframe{New coqdoc}{Yann Régis-Gianas}
...
\begin{subsecframe}{\texttt{opam} -- \name{T. Braibant}}
  ...
  \begin{itemize}
  \item \name{Coq, HoTT/Coq, Coq+Ssreflect}, any version, git included
...
\begin{subsecframe}{Tentative planning}
  \begin{itemize}
  \item Late February 2014: 8.5-α release and 8.4pl6 (or 8.4-2) (\TODO{MS:n bugs closed})
...
\begin{subsecframe}{HoTT or OTT?}
  ...
  \TODO{MS: split, give developer/user high-level views}
  \Coq's position as a compromise.

  HoTT ideas, math oriented:
  \begin{itemize}
  \item HITs (prototype by \name{B. Barras}, \name{J. Gross} will talk about them next)
...
\begin{subsecframe}{User libraries, plugins}
  ...
  \item \name{Mtac}, \name{Cybele}: ``internalized'' tactic language
    (demo by \name{B.~Ziliani})
  \item \name{Paco}: coinduction, logically
